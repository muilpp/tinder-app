...
public class MatchesRecyclerAdapter extends RecyclerView.Adapter<MatchesRecyclerAdapter.MatchViewHolder> {
    ...
    public MatchesRecyclerAdapter(List<Match> matchList, Activity parentActivity) {
    ...
    public void onBindViewHolder(final MatchViewHolder holder, final int position) {
        if (matchList.get(position).getPerson() == null)
            return;
        ...
        if (!messageList.isEmpty()) {
            ...
            holder.lastMessage.setText(messageList.get(messageList.size()-1).getMessageText());
        } else {
        ...
        holder.cv.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                ...
                chatActivityIntent.putExtra(BuildConfig.USER_ID, matchList.get(holder.getAdapterPosition()).getPerson().getId());
                chatActivityIntent.putExtra(BuildConfig.MATCH_ID, matchList.get(holder.getAdapterPosition()).getId());
                chatActivityIntent.putExtra(BuildConfig.USER_IMAGE, matchList.get(holder.getAdapterPosition()).getPerson().getPhotoList().get(0).getProcessedFiles().get(0).getUrl());
